...
library(rtdists)
library(msm)
source(file = "Modelling/dataset2/02_megaBackground.R")
...
for (useSub in 1:nSub) { #Run DDM for each subject in n Subjects
  ...
  savefile=paste("Modelling/dataset2/07_Outputs/P",useSub,"Simple_Model.Rdata",sep="")
...
for(useSub in 1:nSub) {
  ...
  load(paste("Modelling/dataset2/07_Outputs/P",useSub,"Simple_Model.Rdata", sep = "")) #Loads through the datasets of each participant in nSub
  ...
  save(sim, file = paste("Data/dataset2/Model-Predictions/P",useSub,"_simple.RData", sep = ""))
