...
for (useSub in 1:nSub) {
  ...
  load(here(paste("Data/dataset1a/clean/P",useSub,".Rdata",sep="")))
  ...
  savefile=here(paste("Modelling/dataset1a/07_Outputs/P",useSub,"_v_Model.Rdata",sep=""))
...
for(useSub in 1:nSub) {
  ...
  load(here(paste("Modelling/dataset1a/07_Outputs/P",useSub,"_v_Model.Rdata", sep = ""))) #Loads through the datasets of each participant in nSub
  ...
  for (cond in conds) { # Loops through each cue condition (congruent and incongruent)
    currParams=c(blah["a"],0.5,blah[paste("v", cond, sep = ".")],blah["t0"],blah["sv"],blah["sz"],blah["st0"]) # Sets the value of parameters. 
    names(currParams)=c("a","z","v","t0","t0","sv","sz","st0")  # Sets the names of the parameters
  ...
  save(sim, file = here(paste("Data/dataset1a/Model-Predictions/P",useSub,"_v.Rdata", sep = "")))
