...
for (useSub in 1:nSub) {
  ...
  theta.names=c("a",
                paste("t0",conds,sep="."), "v","sv","sz","st0") 
  
  savefile=here(paste("Modelling/dataset1a/07_Outputs/P",useSub,"_t0_Model.Rdata",sep=""))
...
for(useSub in 1:nSub) {
  ...
  load(here(paste("Modelling/dataset1a/07_Outputs/P",useSub,"_t0_Model.Rdata", sep = ""))) #Loads through the datasets of each participant in nSub
  ...
  for (cond in conds) { # Loops through each cue condition (congruent and incongruent)
    currParams=c(blah["a"],0.5,blah["v"],blah[paste("t0", cond, sep = ".")],blah["sv"],blah["sz"],blah["st0"]) # Sets the value of parameters. 
  ...
  save(sim, file = here(paste("Data/dataset1a/Model-Predictions/P",useSub,"_t0.Rdata", sep = "")))
