...
for (useSub in 1:nSub) {
  ...
  savefile=paste("Modelling/07_Outputs/DS",dataset,"_P",useSub,"_all-params_Model.Rdata",sep="")
  ...
  #weight = likelihoods 
  #burnin = discarded samples from MCMC
  #nmc = ?
  #theta.names = names of parameters
...
for(useSub in 1:41) {
  ...
  load(paste("Modelling/07_Outputs/ds",dataset,"_P",useSub,"_all-params_Model.Rdata", sep = "")) #Loads through the datasets of each participant in nSub
  ...
  save(sim, file = paste("Data/Model-Predictions/DS",dataset,"_P",useSub,"_all-params.Rdata", sep = ""))
