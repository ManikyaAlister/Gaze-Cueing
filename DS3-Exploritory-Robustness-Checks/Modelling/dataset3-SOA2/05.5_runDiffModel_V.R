...
for (useSub in 1:nSub) {
  ...
  savefile=paste("Modelling/dataset3-SOA2/07_Outputs/P",useSub,"_v_Model.Rdata",sep="")
...
for(useSub in 1:nSub) {
  ...
  load(paste("Modelling/dataset3-SOA2/07_Outputs/P",useSub,"_v_Model.Rdata", sep = "")) #Loads through the datasets of each participant in nSub
  ...
  for (cond in conds) { # Loops through each cue condition (congruent and incongruent)
    currParams=c(blah["a"],0.5,blah[paste("v", cond, sep = ".")],blah["t0"]) # Sets the value of parameters. 
  ...
  save(sim, file = paste("Data/dataset3-SOA2/Model-Predictions/P",useSub,"_v.Rdata", sep = ""))
