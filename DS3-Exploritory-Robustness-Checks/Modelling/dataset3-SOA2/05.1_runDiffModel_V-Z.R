...
setwd(here())
source("Modelling/dataset3-SOA2/02_megaBackground.R")
...
for (useSub in 1:nSub) {
  load(paste("Data/dataset3-SOA2/clean/P",useSub,".Rdata",sep=""))
  ...
  savefile=paste("Modelling/dataset3-SOA2/07_Outputs/P",useSub,"_v-z_Model.Rdata",sep="")
  
  source("Modelling/dataset3-SOA2/03_background.R")
  source("Modelling/dataset3-SOA2/04_runIterativeProcess.R")
...
for(useSub in 1:nSub) {
  ...
  load(paste("Modelling/dataset3-SOA2/07_Outputs/P",useSub,"_v-z_Model.Rdata", sep = "")) #Loads through the datasets of each participant in nSub
  ...
  for (cond in conds) { # Loops through each cue condition (congruent and incongruent)
    currParams=c(blah["a"],NA,blah[paste("v", cond, sep = ".")],blah["t0"]) # Sets the value of parameters. 
  ...
  save(sim, file = paste("Data/dataset3-SOA2/Model-Predictions/P",useSub,"_v-z.Rdata", sep = ""))
