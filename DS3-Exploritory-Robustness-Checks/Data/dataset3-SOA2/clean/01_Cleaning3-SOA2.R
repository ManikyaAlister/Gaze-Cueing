...
for (i in rm_error) {
  ...
  save(data, file = paste("Data/dataset3-SOA2/clean/rm-error/P",i,".Rdata",sep=""))
}
...
for (i in 1:9) { 
  ...
  data$PID = i
  data = select(data, PID, name_trial__, RESPONSE_TIME, RESPONSE_ACCURACY, trial_type, gaze_direct,pict3t) #pict3t == 100 ~ SOA = 200, pict3t == 400 ~ SOA = 500
  names(data) = c("PID", "trial", "Time", "Resp", "Validity", "Stim","pict3t")
  data$SOA = case_when(
    data$pict3t == 100 ~ 200,
    data$pict3t == 400 ~ 500)
  data = select(data,-pict3t)
  data = filter(data,SOA == 200)
  ...
  if (acc < .8) {
    save(data, file = paste("Data/dataset3-SOA2/clean/rm-accuracy/P",i,".Rdata",sep=""))
  } else {
    ...
    save(data, file = paste("Data/dataset3-SOA2/clean/gaps/P",i,".Rdata",sep="")) #Makes the modeling annoying if there are gaps in the 
...
for (i in 10:80) { 
  ...
  if (acc < .8) {
    ...
    data = filter(data, Time < 5) #Filter out responses slower than 5 seconds 
    data = filter(data, Time > 0.1)#Filter out responses faster than 0.1 seconds 
    save(data, file = paste("Data/dataset3-SOA2/clean/gaps/P",i,".Rdata",sep=""))
    ntrials_post[i] = length(data$trial)
  }
}
...
for (useSub in 1:nSub) {
  ...
  load(paste("Data/dataset3-SOA2/clean/gaps/P",useSub,".Rdata",sep=""))
...
for (i in 1:71) {
  ...
  save(data, file = paste("Data/dataset3-SOA2/clean/P",i,".Rdata",sep=""))
}
...
SOA = NULL
for (i in 1:71) {
  load(paste("Data/dataset3-SOA2/clean/P",i,".Rdata",sep=""))
  PID = data$PID
  trial = data$trial
  Time = data$Time 
  Resp = data$Resp
  ...
  Cond = data$Cond
  SOA = data$SOA
  p = cbind(PID, trial, Time, Resp, Validity, Stim, Cond, SOA)
...
save(P, file = "Data/dataset3-SOA2/clean/all-participants.Rdata")         
