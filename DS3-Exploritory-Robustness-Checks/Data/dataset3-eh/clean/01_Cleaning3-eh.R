...
for (i in rm_error) {
  ...
  save(data, file = paste("Data/dataset3-eh/clean/rm-error/P",i,".Rdata",sep=""))
}
...
for (i in 1:9) { 
  ...
  data = filter(data,emotion == "happy")
  ...
  if (acc < .8) {
    save(data, file = paste("Data/dataset3-eh/clean/rm-accuracy/P",i,".Rdata",sep=""))
  } else {
    ...
    save(data, file = paste("Data/dataset3-eh/clean/gaps/P",i,".Rdata",sep="")) #Makes the modeling annoying if there are gaps in the 
...
for (i in 10:80) { 
  ...
  if (acc < .8) {
    ...
    data = filter(data, Time < 5) #Filter out responses slower than 5 seconds 
    data = filter(data, Time > 0.1)#Filter out responses faster than 0.1 seconds 
    save(data, file = paste("Data/dataset3-eh/clean/gaps/P",i,".Rdata",sep=""))
    ntrials_post[i] = length(data$trial)
  }
}
...
for (useSub in 1:nSub) {
  ...
  load(paste("Data/dataset3-eh/clean/gaps/P",useSub,".Rdata",sep=""))
...
for (i in 1:71) {
  ...
  save(data, file = paste("Data/dataset3-eh/clean/P",i,".Rdata",sep=""))
}
...
emotion = NULL
for (i in 1:71) {
  load(paste("Data/dataset3-eh/clean/P",i,".Rdata",sep=""))
  PID = data$PID
  trial = data$trial
  Time = data$Time 
  Resp = data$Resp
...
save(P, file = "Data/dataset3-eh/clean/all-participants.Rdata")         
