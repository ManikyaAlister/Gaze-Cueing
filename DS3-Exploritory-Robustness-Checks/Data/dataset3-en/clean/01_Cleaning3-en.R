...
for (i in rm_error) {
  ...
  save(data, file = paste("Data/dataset3-en/clean/rm-error/P",i,".Rdata",sep=""))
}
...
for (i in 1:9) { 
  ...
  data$PID = i
  data = select(data, PID, name_trial__, RESPONSE_TIME, RESPONSE_ACCURACY, trial_type, gaze_direct,emotion) 
  names(data) = c("PID", "trial", "Time", "Resp", "Validity", "Stim","emotion")
  data = filter(data,emotion == "neutral")
  ...
  if (acc < .8) {
    save(data, file = paste("Data/dataset3-en/clean/rm-accuracy/P",i,".Rdata",sep=""))
  } else {
    ...
    save(data, file = paste("Data/dataset3-en/clean/gaps/P",i,".Rdata",sep="")) #Makes the modeling annoying if there are gaps in the 
...
for (i in 10:80) { 
  ...
  data = read.delim(file = paste("Data/dataset3/raw/AT_",i,"_RESULTS_FILE.txt", sep = ""))
  data$PID = i
  data = select(data, PID, name_trial__, RESPONSE_TIME, RESPONSE_ACCURACY, trial_type, gaze_direct,emotion) #emotion == 100 ~ SOA = 200, emotion == 400 ~ SOA = 500
  names(data) = c("PID", "trial", "Time", "Resp", "Validity", "Stim","emotion")
  data = filter(data,emotion == "neutral")
  data$Time = data$Time/1000
  data$Resp = case_when(
  ...
  if (acc < .8) {
    ...
    data = filter(data, Time < 5) #Filter out responses slower than 5 seconds 
    data = filter(data, Time > 0.1)#Filter out responses faster than 0.1 seconds 
    save(data, file = paste("Data/dataset3-en/clean/gaps/P",i,".Rdata",sep=""))
    ntrials_post[i] = length(data$trial)
  }
}
...
#Re-save so that the files are ordered 1-70 (not 71 because extra p removed in neutral condition)
...
for (useSub in 1:nSub) {
  if(useSub %in% c(11, 14, 32, 29, 32, 34, 39, 44, 64, 75, 80)) next # P14 less than 80% accuracy in the neutral face condition
  load(paste("Data/dataset3-en/clean/gaps/P",useSub,".Rdata",sep=""))
...
#Re-save in order 1-70
for (i in 1:70) {
  data = as.data.frame(all.data[i])
  save(data, file = paste("Data/dataset3-en/clean/P",i,".Rdata",sep=""))
}
...
emotion = NULL
for (i in 1:70) {
  load(paste("Data/dataset3-en/clean/P",i,".Rdata",sep=""))
  PID = data$PID
  trial = data$trial
  Time = data$Time 
  Resp = data$Resp
  ...
  p = cbind(PID, trial, Time, Resp, Validity, Stim, Cond, emotion)
...
save(P, file = "Data/dataset3-en/clean/all-participants.Rdata")         
