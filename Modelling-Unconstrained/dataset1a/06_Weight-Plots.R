...
for (p in 1:S) { 
  ...
  AIC_t0[p] = AIC
  BIC_t0[p] = BIC
  t0_diff[p] = apply(theta, 2, mean)["t0.1"] - apply(theta, 2, mean)["t0.2"]
}
save(t0_diff, file = "Data/dataset1a/derived-unconstrained/t0_diff.Rdata")
#---------------------------------------------------------------------------------------------------
## z_t0 model##
...
for (p in 1:S) { #Loop in each data set
  ...
  AIC_C[p] = AIC
  BIC_C[p] = BIC
  t0_diff_c[p] = apply(theta, 2, mean)["t0.1"] - apply(theta, 2, mean)["t0.2"]
}
...
#To get an inclusion probability for z, we just add the models that have z and don’t have z:
...
for (i in 1:S) {
  ...
  rect(i-0.5,0,i+0.5,currWeight, border = col, col = col)
  sumThing=sumThing+currWeight
  col = ifelse (t0_diff[i] >= 0, "red", "green")
...
#### t0 #####
#And same principle for t0:
png("Modelling-Unconstrained/dataset1a/08_Plots/inclusion-prob-BIC-t0-red-complex.png") 
plot(x=100,y=100,xlim=c(0,S),ylim=c(0,1),xlab="",ylab="",main="Complex t0 Inclusion Probability BIC",xaxt="n",yaxt="n")
for (i in 1:S) {
  use.i=i
  sumThing=0
  col=hcl(h=330,c=100,l=0)
  ...
  rect(i-0.5,0,i+0.5,currWeight, border = col, col = col)
  sumThing=sumThing+currWeight
  col = ifelse (t0_diff_c[i] > 0, "red", "green")
  currWeight=sum(BICweights[use.i,c("t0","z_t0", "v_t0", "complex")])
  rect(i-0.5,sumThing,i+0.5,sumThing+currWeight, border = col, col = col)
  sumThing=sumThing+currWeight
}
