...
save(BIC_incl_prob, file = "Data/dataset1c/derived/BIC_inclusion_probabilities.RData")

#Get gaze cueing magnitudes
library(tidyverse)
load("Data/dataset1c/clean/all-participants.RData")
data = P
data = data %>%
  select(ID, Time, Cond) %>%
  group_by(ID,Cond) %>%
...
for (i in 1:S) {
  load(paste("Modelling/dataset1c/07_Outputs/P",i,"_complex_Model.RData", sep = "")) 
...
table = cbind(magnitude, params.diff, prob_z, prob_v, prob_t0)
save(table, file = "Data/dataset1c/derived/magnitudes-params-incl.probs.RData")
